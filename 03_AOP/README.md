...
- 애플리케이션의 여러 부분에 걸쳐 있는 기능을 횡단 관심사 (Cross-cuttion concerns)라고 한다
...
- AspectJ 애노테이션을 적용을 위해서는 설정 파일에 아래와 깉이 프록시 설정을 해야 한다.
...
public class 클래스명 {
    ...
    @Around("포인트컷 지정자")
    public String around(ProcedingJoinPoint joinPoint) {
        // 메소드 호출 이전, 이후, 예외 발생 등 모든 시점에 적용가능한 어드바이스를 정의
    }
}
```
